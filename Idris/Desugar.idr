...
bindFun fc ns ma f =
  ...
  Elaborable_Apply fc (Elaborable_Apply fc (Elaborable_Name fc (addNS ns $ UN $ Basic ">>=")) ma) f

seqFun : FC -> Maybe Namespace -> RawImp -> RawImp -> RawImp
seqFun fc ns ma mb =
  let fc = virtualiseFC fc in
  Elaborable_Apply fc (Elaborable_Apply fc (Elaborable_Name fc (addNS ns (UN $ Basic ">>"))) ma) mb
...
bindBangs ((n, fc, btm) :: bs) ns tm
    = bindBangs bs ns
    $ bindFun fc ns btm
    $ Elaborable_Lambda EmptyFC top Explicit (Just n) (Implicit fc False) tm

idiomise : FC -> Maybe Namespace -> Maybe Namespace -> RawImp -> RawImp
idiomise fc dons mns (Elaborable_Alternative afc u alts)
  = Elaborable_Alternative afc (mapAltType (idiomise afc dons mns) u) (idiomise afc dons mns <$> alts)
idiomise fc dons mns (Elaborable_Apply afc f a)
  = let fc  = virtualiseFC fc
        app = UN $ Basic "<*>"
        nm  = maybe app (`NS` app) (mns <|> dons)
     in Elaborable_Apply fc (Elaborable_Apply fc (Elaborable_Name fc nm) (idiomise afc dons mns f)) a
idiomise fc dons mns fn
  = let fc  = virtualiseFC fc
        pur = UN $ Basic "pure"
        nm  = maybe pur (`NS` pur) (mns <|> dons)
     in Elaborable_Apply fc (Elaborable_Name fc nm) fn
...
mutual
  ...
  desugarB side ps (PRef fc x) = do
    ...
    case x == pur of -- implicitly add namespace to unqualified occurrences of `pure` in a qualified do-block
      False => pure $ Elaborable_Name fc x
      True => pure $ Elaborable_Name fc (maybe pur (`NS` pur) ns)
  ...
  desugarB side ps (Forall (MkWithData _ (names, scope)))
      ...
      where
        ...
        desugarForallNames ctx (x :: xs)
          = Elaborable_Dependent_Function_Type x.fc erased Implicit (Just x.val)
      ...
      where
        ...
        desugarMultiBinder ctx (name :: xs)
          = let extendedCtx = name.val :: ps
            in Elaborable_Dependent_Function_Type binder.fc rig
  ...
  desugarB side ps (PPi fc rig p mn argTy retTy)
      = let ps' = maybe ps (:: ps) mn in
            pure $ Elaborable_Dependent_Function_Type fc rig !(traverse (desugar side ps') p)
  ...
  desugarB side ps (PLam fc rig p pat@(PRef prefFC n@(UN nm)) argTy scope)
      =  if isPatternVariable nm
           then do whenJust (isConcreteFC prefFC) $ \nfc
                   ...
                   pure $ Elaborable_Lambda fc rig !(traverse (desugar AnyExpr ps) p)
                           (Just n) !(desugarB AnyExpr ps argTy)
                                    !(desugar AnyExpr (n :: ps) scope)
           else pure $ Elaborable_Lambda EmptyFC rig !(traverse (desugar AnyExpr ps) p)
                   (Just (MN "lamc" 0)) !(desugarB AnyExpr ps argTy) $
                 Elaborable_Case fc [] (Elaborable_Name EmptyFC (MN "lamc" 0)) (Implicit fc False)
  ...
  desugarB side ps (PLam fc rig p pat argTy scope)
      = pure $ Elaborable_Lambda EmptyFC rig !(traverse (desugar AnyExpr ps) p)
                   (Just (MN "lamc" 0)) !(desugarB AnyExpr ps argTy) $
                 Elaborable_Case fc [] (Elaborable_Name EmptyFC (MN "lamc" 0)) (Implicit fc False)
                     [snd !(desugarClause ps True (MkPatClause fc pat scope []))]
  desugarB side ps (PLet fc rig (PRef prefFC n) nTy nVal scope [])
      = do whenJust (isConcreteFC prefFC) $ \nfc =>
             addSemanticDecorations [(nfc, Bound, Just n)]
           pure $ Elaborable_Binding fc prefFC rig n !(desugarB side ps nTy) !(desugarB side ps nVal)
                                       !(desugar side (n :: ps) scope)
  desugarB side ps (PLet fc rig pat nTy nVal scope alts)
      = pure $ Elaborable_Case fc [] !(desugarB side ps nVal) !(desugarB side ps nTy)
  ...
  desugarB side ps (PCase fc opts scr cls)
      = do opts <- traverse (desugarFnOpt ps) opts
           ...
           pure $ Elaborable_Case fc opts scr scrty cls
  desugarB side ps (PLocal fc xs scope)
      = let ps' = definedIn (map val xs) ++ ps in
            pure $ Elaborable_Local_Definitions fc (concat !(traverse (desugarDecl ps') xs))
                             !(desugar side ps' scope)
  desugarB side ps (PApp pfc (PUpdate fc fs) rec)
      = pure $ Elaborable_Record_Update pfc !(traverse (desugarUpdate side ps) fs)
  ...
  desugarB side ps (PApp fc x y)
      = pure $ Elaborable_Apply fc !(desugarB side ps x) !(desugarB side ps y)
  desugarB side ps (PAutoApp fc x y)
      = pure $ Elaborable_Automatic_Apply fc !(desugarB side ps x) !(desugarB side ps y)
  desugarB side ps (PWithApp fc x y)
      = pure $ Elaborable_With_Apply fc !(desugarB side ps x) !(desugarB side ps y)
  desugarB side ps (PNamedApp fc x argn y)
      = pure $ Elaborable_Named_Apply fc !(desugarB side ps x) argn !(desugarB side ps y)
  desugarB side ps (PDelayed fc r ty)
      = pure $ Elaborable_Delayed_Type fc r !(desugarB side ps ty)
  desugarB side ps (PDelay fc tm)
      = pure $ Elaborable_Delay fc !(desugarB side ps tm)
  desugarB side ps (PForce fc tm)
      = pure $ Elaborable_Force fc !(desugarB side ps tm)
  desugarB side ps (PEq fc l r)
      = do l' <- desugarB side ps l
           r' <- desugarB side ps r
           pure $ Elaborable_Alternative fc FirstSuccess
                     [apply (Elaborable_Name fc (UN $ Basic "===")) [l', r'],
                      apply (Elaborable_Name fc (UN $ Basic "~=~")) [l', r']]
  ...
  desugarB side ps (PPrimVal fc (BI x))
      = case !fromIntegerName of
             Nothing =>
                pure $ Elaborable_Alternative fc (UniqueDefault (Elaborable_Primitive_Value fc (BI x)))
                                [Elaborable_Primitive_Value fc (BI x),
                                 Elaborable_Primitive_Value fc (I (fromInteger x))]
             Just fi =>
               let vfc = virtualiseFC fc in
               pure $ Elaborable_Apply vfc (Elaborable_Name vfc fi) (Elaborable_Primitive_Value fc (BI x))
  desugarB side ps (PPrimVal fc (Ch x))
      = case !fromCharName of
             Nothing =>
                pure $ Elaborable_Primitive_Value fc (Ch x)
             Just f =>
               let vfc = virtualiseFC fc in
               pure $ Elaborable_Apply vfc (Elaborable_Name vfc f) (Elaborable_Primitive_Value fc (Ch x))
  desugarB side ps (PPrimVal fc (Db x))
      = case !fromDoubleName of
             Nothing =>
                pure $ Elaborable_Primitive_Value fc (Db x)
             Just f =>
               let vfc = virtualiseFC fc in
               pure $ Elaborable_Apply vfc (Elaborable_Name vfc f) (Elaborable_Primitive_Value fc (Db x))
  desugarB side ps (PPrimVal fc x) = pure $ Elaborable_Primitive_Value fc x
  desugarB side ps (PQuote fc tm)
      = do let q = Elaborable_Quote fc !(desugarB side ps tm)
           case side of
                AnyExpr => pure $ maybeIApp fc !fromTTImpName q
                _ => pure q
  desugarB side ps (PQuoteName fc n)
      = do let q = Elaborable_Quote_Name fc n
           case side of
                AnyExpr => pure $ maybeIApp fc !fromNameName q
                _ => pure q
  desugarB side ps (PQuoteDecl fc x)
      = do xs <- traverse (desugarDecl ps) x
           let dls = Elaborable_Quote_Declarations fc (concat xs)
           case side of
                AnyExpr => pure $ maybeIApp fc !fromDeclsName dls
                _ => pure dls
  desugarB side ps (PUnquote fc tm)
      = pure $ Elaborable_Unquote fc !(desugarB side ps tm)
  desugarB side ps (PRunElab fc tm)
      = pure $ Elaborable_Run_Elaborator fc True !(desugarB side ps tm)
  desugarB side ps (PHole fc br holename)
      = do when br $ update Syn { bracketholes $= ((UN (Basic holename)) ::) }
           pure $ Elaborable_Hole fc holename
  desugarB side ps (PType fc) = pure $ Elaborable_Type_Universe fc
  desugarB side ps (PAs fc nameFC vname pattern)
      = pure $ Elaborable_As_Pattern fc nameFC UseRight vname !(desugarB side ps pattern)
  desugarB side ps (PDotted fc x)
      = pure $ Elaborable_Must_Unify fc UserDotted !(desugarB side ps x)
  ...
  desugarB side ps (PString fc hashtag [])
      = pure $ maybeIApp fc !fromStringName (Elaborable_Primitive_Value fc (Str ""))
  desugarB side ps (PString fc hashtag [StrLiteral fc' str])
      = case unescape hashtag str of
             Just str => pure $ maybeIApp fc !fromStringName (Elaborable_Primitive_Value fc' (Str str))
  ...
  desugarB side ps (PBang fc term)
      = do itm <- desugarB side ps term
           ...
           pure (Elaborable_Name (virtualiseFC fc) bn)
  ...
  desugarB side ps (PPair fc l r)
      = do l' <- desugarB side ps l
           r' <- desugarB side ps r
           let pval = apply (Elaborable_Name fc mkpairname) [l', r']
           pure $ Elaborable_Alternative fc (UniqueDefault pval)
                  [apply (Elaborable_Name fc pairname) [l', r'], pval]
  desugarB side ps (PDPair fc opFC (PRef nameFC n@(UN _)) (PImplicit _) r)
      = do r' <- desugarB side ps r
           let pval = apply (Elaborable_Name opFC mkdpairname) [Elaborable_Name nameFC n, r']
           let vfc = virtualiseFC nameFC
           whenJust (isConcreteFC nameFC) $ \nfc =>
             addSemanticDefault (nfc, Bound, Just n)
           pure $ Elaborable_Alternative fc (UniqueDefault pval)
                  [apply (Elaborable_Name opFC dpairname)
                      [Implicit vfc False,
                       Elaborable_Lambda nameFC top Explicit (Just n) (Implicit vfc False) r'],
                   pval]
  desugarB side ps (PDPair fc opFC (PRef namefc n@(UN _)) ty r)
      = do ty' <- desugarB side ps ty
           r' <- desugarB side ps r
           pure $ apply (Elaborable_Name opFC dpairname)
                        [ty', Elaborable_Lambda namefc top Explicit (Just n) ty' r']
  desugarB side ps (PDPair fc opFC l (PImplicit _) r)
      = do l' <- desugarB side ps l
           r' <- desugarB side ps r
           pure $ apply (Elaborable_Name opFC mkdpairname) [l', r']
  desugarB side ps (PDPair fc opFC l ty r)
      = throw (GenericMsg fc "Invalid dependent pair type")
  desugarB side ps (PUnit fc)
      = pure $ Elaborable_Alternative fc (UniqueDefault (Elaborable_Name fc (UN $ Basic "MkUnit")))
               [Elaborable_Name fc (UN $ Basic "Unit"),
                Elaborable_Name fc (UN $ Basic "MkUnit")]
  desugarB side ps (PIfThenElse fc x t e)
      = let fc = virtualiseFC fc in
        pure $ Elaborable_Case fc [] !(desugarB side ps x) (Elaborable_Name fc (UN $ Basic "Bool"))
                   [PatClause fc (Elaborable_Name fc (UN $ Basic "True")) !(desugar side ps t),
                    PatClause fc (Elaborable_Name fc (UN $ Basic "False")) !(desugar side ps e)]
  ...
  desugarB side ps (PRewrite fc rule tm)
      = pure $ Elaborable_Rewrite fc !(desugarB side ps rule) !(desugarB side ps tm)
  ...
  desugarB side ps (PUnifyLog fc lvl tm)
      = pure $ Elaborable_Unification_Log fc lvl !(desugarB side ps tm)
  ...
  desugarB side ps (PWithUnambigNames fc ns rhs)
      = Elaborable_With_Unambiguous_Names fc ns <$> desugarB side ps rhs
  ...
  desugarUpdate : {auto s : Ref Syn SyntaxInfo} ->
                  ...
                  {auto m : Ref MD Metadata} ->
                  {auto o : Ref ROpts REPLOpts} ->
                  Side -> List Name -> PFieldUpdate -> Core Elaborable_Field_Update
  desugarUpdate side ps (PSetField p v)
      = pure (Elaborable_Set_Field p !(desugarB side ps v))
  desugarUpdate side ps (PSetFieldApp p v)
      = pure (Elaborable_Apply_To_Field p !(desugarB side ps v))
  ...
  expandList side ps nilFC [] = pure (Elaborable_Name nilFC (UN $ Basic "Nil"))
  expandList side ps nilFC ((consFC, x) :: xs)
      = pure $ apply (Elaborable_Name consFC (UN $ Basic "::"))
  ...
  expandSnocList side ps nilFC [<] = pure (Elaborable_Name nilFC (UN $ Basic "Lin"))
  ...
  maybeIApp fc nm tm
      = case nm of
             ...
             Just f =>
               let fc = virtualiseFC fc in
               Elaborable_Apply fc (Elaborable_Name fc f) tm
  ...
  expandString side ps fc hashtag xs
    = do xs <- traverse toRawImp (filter notEmpty $ mergeStrLit xs)
         pure $ case xs of
           [] => Elaborable_Primitive_Value fc (Str "")
           (_ :: _) =>
             let vfc = virtualiseFC fc in
             Elaborable_Apply vfc
               (Elaborable_Named_Apply vfc
                 (Elaborable_Name vfc (NS preludeNS $ UN $ Basic "concat"))
                 (UN $ Basic "t")
                 (Elaborable_Name vfc (NS preludeNS $ UN $ Basic "List")))
               (strInterpolate xs)
    where
      toRawImp : PStr -> Core RawImp
      toRawImp (StrLiteral fc str) =
        case unescape hashtag str of
             Just str => pure $ Elaborable_Primitive_Value fc (Str str)
             Nothing => throw (GenericMsg fc "Invalid escape sequence: \{show str}")
      ...
      strInterpolate []
        = Elaborable_Name EmptyFC nilName
      strInterpolate (x :: xs)
        = let xFC = virtualiseFC (getFC x) in
          apply (Elaborable_Name xFC consName)
          [ Elaborable_Apply xFC (Elaborable_Name EmptyFC interpolateName)
                     x
          , strInterpolate xs
          ]
  ...
  expandDo side ps topfc ns (DoBind fc nameFC n rig ty tm :: rest)
      = do tm' <- desugarDo side ps ns tm
           ...
           pure $ bindFun fc ns tm'
                $ Elaborable_Lambda nameFC rig Explicit (Just n) ty' rest'
  expandDo side ps topfc ns (DoBindPat fc pat ty exp alts :: rest)
      = do pat' <- desugarDo LHS ps ns pat
           ...
           pure $ bindFun fc ns exp'
                $ Elaborable_Lambda EmptyFC top Explicit (Just (MN "_" 0))
                          ty'
                          (Elaborable_Case fc [] (Elaborable_Name patFC (MN "_" 0))
                               (Implicit fc False)
  ...
  expandDo side ps topfc ns (DoLet fc lhsFC n rig ty tm :: rest)
      = do b <- newRef Bang (initBangs ns)
           ...
           whenJust (isConcreteFC lhsFC) $ \nfc =>
             addSemanticDecorations [(nfc, Bound, Just n)]
           let bind = Elaborable_Binding fc lhsFC rig n ty' tm' rest'
           bd <- get Bang
  ...
  getClauseFn (Elaborable_Name _ n) = pure n
  getClauseFn (Elaborable_Apply _ f _) = getClauseFn f
  getClauseFn (Elaborable_With_Apply _ f _) = getClauseFn f
  getClauseFn (Elaborable_Automatic_Apply _ f _) = getClauseFn f
  getClauseFn (Elaborable_Named_Apply _ f _ _) = getClauseFn f
  ...
  desugarClause ps arg (MkPatClause fc lhs rhs wheres)
      = do ws <- traverse (desugarDecl ps) wheres
           ...
           let rhs' = case ws of
                        [] => rhs'
                        _ => Elaborable_Local_Definitions fc (concat ws) rhs'
  ...
  desugarDecl ps claim@(MkWithData _ (PClaim (MkPClaim rig vis fnopts ty)))
      = do opts <- traverse (desugarFnOpt ps) fnopts
           ...
           pure $ flip (map {f = List, b = ImpDecl}) types $ \ty' =>
                      Elaborable_Claim (MkFCVal claim.fc $ Make_Elaborable_Claim_Data rig vis opts ty')
  ...
  -- after desugaring, by function name, using collectDefs from RawImp
    ...
    where
      ...
      toIDef nm (PatClause fc lhs rhs)
          = pure $ Elaborable_Definition fc nm [PatClause fc lhs rhs]
      toIDef nm (WithClause fc lhs rig rhs prf flags cs)
          = pure $ Elaborable_Definition fc nm [WithClause fc lhs rig rhs prf flags cs]
      toIDef nm (ImpossibleClause fc lhs)
          = pure $ Elaborable_Definition fc nm [ImpossibleClause fc lhs]

  desugarDecl ps dat@(MkWithData _ $ PData doc vis mbtot ddecl)
      = pure [Elaborable_Data_Declaration dat.fc vis mbtot !(desugarData ps doc ddecl)]
  ...
  desugarDecl ps pp@(MkWithData _ $ PParameters params pds)
      = do
           ...
           pure [Elaborable_Parameter_Block pp.fc paramsb (concat pds')]
      where
  ...
  desugarDecl ps int@(MkWithData _ $ PInterface vis cons_in tn doc params det conname body)
      = do addDocString tn doc
           ...
           pure [Elaborable_Pragma int.fc (maybe [tn] (\n => [tn, n.val]) conname)
                            (\nest, env =>
  ...
  desugarDecl ps impl@(MkWithData _ $ PImplementation vis fnopts pass is cons tn params impln nusing body)
      = do opts <- traverse (desugarFnOpt ps) fnopts
           ...
           pure [Elaborable_Pragma impl.fc [impname]
                            (\nest, env =>
  ...
  desugarDecl ps d@(MkWithData _ $ PFail mmsg ds)
      = do -- save the state: the content of a failing block should be discarded
           ...
           case the (Either (Maybe Error) (List ImpDecl)) result of
             Right ds => [Elaborable_Expected_Failure d.fc mmsg ds] <$ log "desugar.failing" 20 "Success"
  ...
  desugarDecl ps n@(MkWithData _ $ PNamespace ns decls)
      = withExtendedNS ns $ do
           ...
           pure [Elaborable_Namespace_Block n.fc ns (concat ds)]
  desugarDecl ps ts@(MkWithData _ $ PTransform n lhs rhs)
      = do (bound, blhs) <- bindNames False !(desugar LHS ps lhs)
           rhs' <- desugar AnyExpr (bound ++ ps) rhs
           pure [Elaborable_Transformation ts.fc (UN $ Basic n) blhs rhs']
  desugarDecl ps el@(MkWithData _ $ PRunElabDecl tm)
      = do tm' <- desugar AnyExpr ps tm
           pure [Elaborable_Run_Elaborator_Declaration el.fc tm']
  desugarDecl ps dir@(MkWithData _ $ PDirective d)
      = let fc = dir.fc in case d of
             Hide (HideName n) => pure [Elaborable_Pragma fc [] (\nest, env => hide fc n)]
             Hide (HideFixity fx n) => pure [Elaborable_Pragma fc [] (\_, _ => removeFixity fc fx n)]
             Unhide n => pure [Elaborable_Pragma fc [] (\nest, env => unhide fc n)]
             Logging i => pure [Elaborable_Logging ((\ i => (topics i, verbosity i)) <$> i)]
             LazyOn a => pure [Elaborable_Pragma fc [] (\nest, env => lazyActive a)]
             UnboundImplicits a => do
               setUnboundImplicits a
               pure [Elaborable_Pragma fc [] (\nest, env => setUnboundImplicits a)]
             PrefixRecordProjections b => do
               pure [Elaborable_Pragma fc [] (\nest, env => setPrefixRecordProjections b)]
             AmbigDepth n => pure [Elaborable_Pragma fc [] (\nest, env => setAmbigLimit n)]
             TotalityDepth n => pure [Elaborable_Pragma fc [] (\next, env => setTotalLimit n)]
             AutoImplicitDepth n => pure [Elaborable_Pragma fc [] (\nest, env => setAutoImplicitLimit n)]
             NFMetavarThreshold n => pure [Elaborable_Pragma fc [] (\nest, env => setNFThreshold n)]
             SearchTimeout n => pure [Elaborable_Pragma fc [] (\nest, env => setSearchTimeout n)]
             PairNames ty f s => pure [Elaborable_Pragma fc [] (\nest, env => setPair fc ty f s)]
             RewriteName eq rw => pure [Elaborable_Pragma fc [] (\nest, env => setRewrite fc eq rw)]
             PrimInteger n => pure [Elaborable_Pragma fc [] (\nest, env => setFromInteger n)]
             PrimString n => pure [Elaborable_Pragma fc [] (\nest, env => setFromString n)]
             PrimChar n => pure [Elaborable_Pragma fc [] (\nest, env => setFromChar n)]
             PrimDouble n => pure [Elaborable_Pragma fc [] (\nest, env => setFromDouble n)]
             PrimTTImp n => pure [Elaborable_Pragma fc [] (\nest, env => setFromTTImp n)]
             PrimName n => pure [Elaborable_Pragma fc [] (\nest, env => setFromName n)]
             PrimDecls n => pure [Elaborable_Pragma fc [] (\nest, env => setFromDecls n)]
             CGAction cg dir => pure [Elaborable_Pragma fc [] (\nest, env => addDirective cg dir)]
             Names n ns => pure [Elaborable_Pragma fc [] (\nest, env => addNameDirective fc n ns)]
             StartExpr tm => pure [Elaborable_Pragma fc [] (\nest, env => throw (InternalError "%start not implemented"))] -- TODO!
             Overloadable n => pure [Elaborable_Pragma fc [] (\nest, env => setNameFlag fc n Overloadable)]
             Extension e => pure [Elaborable_Pragma fc [] (\nest, env => setExtension e)]
             DefaultTotality tot => pure [Elaborable_Pragma fc [] (\_, _ => setDefaultTotalityOption tot)]
             ForeignImpl n cs => do
  ...
  desugarDecl ps bt@(MkWithData _ $ PBuiltin type name) = pure [Elaborable_Builtin_Declaration bt.fc type name]
