...
The compiler exposes a deliberately small, versioned handoff for native
backends:

```sh
./edric --emit-one-step examples/compiler-one-step/PrintX.idric -o build/PrintX.one-step
```

This command runs the ordinary parser, elaborator, and core checker, then asks
the compiler itself for its ANF one-step-at-a-time form.  The resulting
`EDRIC_ONE_STEP` artifact records the exact source and compiler revisions and a
digest of the compiler-emitted definitions.  It is not generated by matching
source text and it is not a target-language program.  A consumer must reject
operations it does not implement; the emitter does not route around a native
backend through RefC, generated C, or a target Chez program.

The `idris2-anf-show-0.8.0` representation name deliberately pins the current
compiler serialization seam.  It may be replaced by a more stable structured
encoding later, but x86 work does not redefine the compiler form.
...
- Compiler-owned, deterministic checked one-step-at-a-time emitter: implemented.
- Canonical scalar native-backend handoff fixture: `examples/compiler-one-step/PrintX.idric`.
