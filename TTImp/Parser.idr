...
atom fname
    = do start <- location
         ...
         pure (Elaborable_Primitive_Value (MkFC fname start end) x)
  <|> do start <- location
         str <- simpleStr
         end <- location
         pure (Elaborable_Primitive_Value (MkFC fname start end) (Str str))
  <|> do start <- location
         exactIdent "Type"
         end <- location
         pure (Elaborable_Type_Universe (MkFC fname start end))
  <|> do start <- location
         symbol "_"
         end <- location
  ...
  <|> do start <- location
         ...
         pure (Elaborable_Search (MkFC fname start end) 1000)
  <|> do start <- location
         x <- name
         end <- location
         pure (Elaborable_Name (MkFC fname start end) x)
  <|> do start <- location
         symbol "$"
         x <- userName
         end <- location
         pure (Elaborable_Bind_Name (MkFC fname start end) x)
  <|> do start <- location
         x <- holeName
         end <- location
         pure (Elaborable_Hole (MkFC fname start end) x)
...
mutual
  ...
  appExpr fname indents
    ...
    where
      ...
      applyExpImp start end f (Left exp :: args)
          = applyExpImp start end (Elaborable_Apply (MkFC fname start end) f exp) args
      applyExpImp start end f (Right (Just n, imp) :: args)
          = applyExpImp start end (Elaborable_Named_Apply (MkFC fname start end) f n imp) args
      applyExpImp start end f (Right (Nothing, imp) :: args)
          = applyExpImp start end (Elaborable_Automatic_Apply (MkFC fname start end) f imp) args
      ...
      = do start <- location
           ...
           (do symbol "="
             ...
             <|> (do symbol "}"
                     ...
                     pure (Just x, Elaborable_Name (MkFC fname start end) x))
    <|> do symbol "@{"
           commit
           tm <- expr fname indents
           symbol "}"
  ...
  as fname indents
      = do start <- location
           ...
           pure (Elaborable_As_Pattern (MkFC fname start end) (MkFC fname start nameEnd) UseRight x pat)
  ...
  pibindAll fc p (ty :: rest) scope
           = Elaborable_Dependent_Function_Type fc ty.rig p (map val ty.mName) ty.val (pibindAll fc p rest scope)
  ...
  lam fname indents
     ...
     where
       ...
       bindAll fc ((rig, n, ty) :: rest) scope
           = Elaborable_Lambda fc rig Explicit (Just n) ty (bindAll fc rest scope)
  ...
  let_ fname indents
      = do start <- location
           ...
           pure (let fc = MkFC fname start end in
                     Elaborable_Binding fc (boundToFC fname n) rig n.val (Implicit fc False) val scope)
    <|> do start <- location
           keyword "let"
           ...
           pure (Elaborable_Local_Definitions (MkFC fname start end) (collectDefs ds) scope)
  ...
  case_ fname indents
      = do start <- location
           ...
           pure (let fc = MkFC fname start end in
                     Elaborable_Case fc opts scr (Implicit fc False) alts)
  ...
  record_ fname indents
      = do start <- location
           ...
           pure (Elaborable_Record_Update (MkFC fname start end) (forget fs) sc)

  field : OriginDesc -> IndentInfo -> Rule Elaborable_Field_Update
  field fname indents
      = do path <- sepBy1 (symbol "->") unqualifiedName
           upd <- (do symbol "="; pure Elaborable_Set_Field)
                      <|>
                  (do symbol "$="; pure Elaborable_Apply_To_Field)
  ...
  rewrite_ fname indents
      = do start <- location
           ...
           pure (Elaborable_Rewrite (MkFC fname start end) rule tm)
  ...
  lazy fname indents
      = do start <- location
           ...
           pure (Elaborable_Delayed_Type (MkFC fname start end) LLazy tm)
    <|> do start <- location
           exactIdent "Inf"
           tm <- simpleExpr fname indents
           end <- location
           pure (Elaborable_Delayed_Type (MkFC fname start end) LInf tm)
    <|> do start <- location
           exactIdent "Delay"
           tm <- simpleExpr fname indents
           end <- location
           pure (Elaborable_Delay (MkFC fname start end) tm)
    <|> do start <- location
           exactIdent "Force"
           tm <- simpleExpr fname indents
           end <- location
           pure (Elaborable_Force (MkFC fname start end) tm)
  ...
  typeExpr fname indents
    ...
    where
      ...
      mkPi start end arg ((exp, a) :: as)
            = Elaborable_Dependent_Function_Type (MkFC fname start end) top exp Nothing arg
                  (mkPi start end a as)
...
mutual
  ...
  parseRHS withArgs fname indents start lhs
    ...
    where
      ...
      getFn (Elaborable_Name _ n) = pure n
      getFn (Elaborable_Apply _ f a) = getFn f
      getFn (Elaborable_Automatic_Apply _ f a) = getFn f
      getFn (Elaborable_Named_Apply _ f _ a) = getFn f
  ...
  clause withArgs fname indents
    ...
    where
      ...
      applyArgs f [] = f
      applyArgs f ((fc, a) :: args) = applyArgs (Elaborable_Apply fc f a) args
...
definition fname indents
    = do start <- location
         ...
         pure (Elaborable_Definition (MkFC fname start end) (fst nd) [snd nd])
...
fieldDecl : OriginDesc -> IndentInfo -> Rule (List Elaborable_Field)
fieldDecl fname indents
    ...
    <|> do fs <- fieldBody Explicit
           ...
           pure fs
  where
    fieldBody : PiInfo RawImp -> Rule (List Elaborable_Field)
    fieldBody p
        = do start <- location
             ns <- sepBy1 (symbol ",") (withFC userName)
             symbol ":"
...
recordDecl fname indents
    = do start <- location
         ...
         pure (let fc = MkFC fname start end
                in Elaborable_Record_Declaration fc Nothing vis mbtot
...
directive fname indents
  ...
  <|> do b <- bounds (do pragma "builtin"
                         ...
                         pure (t, n))
         (t, n) <- pure b.val
         pure $ Elaborable_Builtin_Declaration (boundToFC fname b) t n

         {- Can't do Elaborable_Pragma due to lack of Ref Ctxt. Should we worry about this?
  <|> do pragma "pair"
         commit
         start <- location
         p <- name
  ...
  <|> do pragma "rewrite"
         ...
         pure (let fc = MkFC fname start end in
                   Elaborable_Pragma (\c, nest, env => setRewrite {c} fc eq rw))
    -}
-- Declared at the top
-- topDecl : OriginDesc -> IndentInfo -> Rule ImpDecl
topDecl fname indents
    = do start <- location
         ...
         pure (Elaborable_Data_Declaration (MkFC fname start end) vis mbtot dat)
  <|> do start <- location
         ns <- namespaceDecl
         ds <- assert_total (nonEmptyBlock (topDecl fname))
         end <- location
         pure (Elaborable_Namespace_Block (MkFC fname start end) ns (forget ds))
  <|> do start <- location
         visOpts <- many visOpt
         ...
         pure (Elaborable_Claim (MkFCVal (MkFC fname start end) $ Make_Elaborable_Claim_Data  rig vis opts claim))
  <|> recordDecl fname indents
  <|> directive fname indents
  <|> definition fname indents
...
-- collectDefs : List ImpDecl -> List ImpDecl
collectDefs [] = []
collectDefs (Elaborable_Definition loc fn cs :: ds)
    = let (cs', rest) = spanMap (isClause fn) ds in
          Elaborable_Definition loc fn (cs ++ cs') :: assert_total (collectDefs rest)
  where
    ...
    isClause n (Elaborable_Definition _ n' cs)
        = if n == n' then Just cs else Nothing
    isClause n _ = Nothing
collectDefs (Elaborable_Namespace_Block loc ns nds :: ds)
    = Elaborable_Namespace_Block loc ns (collectDefs nds) :: collectDefs ds
collectDefs (Elaborable_Expected_Failure loc msg nds :: ds)
    = Elaborable_Expected_Failure loc msg (collectDefs nds) :: collectDefs ds
collectDefs (d :: ds)
    = d :: collectDefs ds
