...
export
isPrimitiveValue : RawImp' nm -> Maybe Constant
isPrimitiveValue (Elaboratable_Primitive_Value _ c) = Just c
isPrimitiveValue _ = Nothing
...
export
findNamesToBind : RawImp' nm -> List String
findNamesToBind (Elaboratable_Dependent_Function_Type fc rig p mn aty retty)
    = findNamesToBind aty ++ findNamesToBind retty
findNamesToBind (Elaboratable_Lambda fc rig p n aty sc)
    = findNamesToBind aty ++ findNamesToBind sc
findNamesToBind (Elaboratable_Apply fc fn av)
    = findNamesToBind fn ++ findNamesToBind av
findNamesToBind (Elaboratable_Automatic_Apply fc fn av)
    = findNamesToBind fn ++ findNamesToBind av
findNamesToBind (Elaboratable_Named_Apply _ fn _ av)
    = findNamesToBind fn ++ findNamesToBind av
findNamesToBind (Elaboratable_With_Apply fc fn av)
    = findNamesToBind fn ++ findNamesToBind av
findNamesToBind (Elaboratable_As_Pattern fc _ _ (UN (Basic n)) pat)
    = n :: findNamesToBind pat
findNamesToBind (Elaboratable_As_Pattern fc _ _ n pat)
    = findNamesToBind pat
findNamesToBind (Elaboratable_Must_Unify fc r pat)
    = findNamesToBind pat
findNamesToBind (Elaboratable_Alternative fc u alts)
    = concatMap findNamesToBind alts
findNamesToBind (Elaboratable_Delayed_Type fc _ ty) = findNamesToBind ty
findNamesToBind (Elaboratable_Delay fc tm) = findNamesToBind tm
findNamesToBind (Elaboratable_Force fc tm) = findNamesToBind tm
findNamesToBind (Elaboratable_Quote fc tm) = findNamesToBind tm
findNamesToBind (Elaboratable_Unquote fc tm) = findNamesToBind tm
findNamesToBind (Elaboratable_Run_Elaborator fc _ tm) = findNamesToBind tm
findNamesToBind (Elaboratable_Bind_Here _ _ tm) = findNamesToBind tm
findNamesToBind (Elaboratable_Bind_Name _ (UN (Basic n))) = [n]
findNamesToBind (Elaboratable_Record_Update fc updates tm)
    = findNamesToBind tm ++ concatMap (findNamesToBind . getFieldUpdateTerm) updates
...
implicitsAs n defs ns tm
  = do let implicits = findNamesToBind tm
...
isIVar (Elaboratable_Name fc v) = Just (fc, v)
isIVar _ = Nothing

export
isIBindVar : RawImp' nm -> Maybe (FC, Name)
isIBindVar (Elaboratable_Bind_Name fc v) = Just (fc, v)
