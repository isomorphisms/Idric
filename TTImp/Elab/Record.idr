...
toLHS' loc (Field mn@(Just _) n _)
    = (mn, Elaborable_As_Pattern loc (virtualiseFC loc) UseRight (UN $ Basic n) (Implicit loc True))
toLHS' loc (Field mn n _) = (mn, Elaborable_Bind_Name (virtualiseFC loc) (UN $ Basic n))
toLHS' loc (Constr mn con args)
    = let args' = map (toLHS' loc . snd) args in
          (mn, gapply (Elaborable_Name loc con) args')
    ...
    mkArgs ((p, imp, _) :: ps) tyArgs
        = do fldn <- genFieldName p
             ...
             let arg = case (flip contains tyArgs) <$> imp of
                  ...
                  _ => Elaborable_Name (virtualiseFC loc) (UN $ Basic fldn)
...
getSides : {auto c : Ref Ctxt Defs} ->
           ...
           Core Rec
getSides loc (Elaborable_Set_Field path val) tyn orig rec
...
getSides loc (Elaborable_Apply_To_Field path val) tyn orig rec
   = findPath loc path path (Just tyn)
      (\n => apply val [Elaborable_Name (virtualiseFC loc) (UN $ Basic n)]) rec

getAllSides : {auto c : Ref Ctxt Defs} ->
              {auto u : Ref UST UState} ->
              FC -> List Elaborable_Field_Update -> Name ->
              RawImp -> Rec ->
              Core Rec
...
recUpdate rigc elabinfo iloc nest env flds rec grecty
      = do let dups = checkForDuplicates flds empty empty
           ...
           sides <- getAllSides iloc flds rectyn rec
                                (Field Nothing fldn (Elaborable_Name vloc (UN $ Basic fldn)))
           pure $ Elaborable_Case vloc [] rec (Implicit vloc False) [mkClause sides]
  where
    vloc : FC
...
checkUpdate : {vars : _} ->
              ...
              RigCount -> ElabInfo ->
              NestedNames vars -> Env Term vars ->
              FC -> List Elaborable_Field_Update -> RawImp -> Maybe (Glued vars) ->
