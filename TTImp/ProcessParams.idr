...
processParams {vars} {c} {m} {u} nest env fc ps ds
    = do -- Turn the parameters into a function type, (x : ps) -> Type,
         ...
         pty_imp <- bindTypeNames fc [] (toList vars) (Elaborable_Bind_Here fc (PI erased) pty_raw)
  ...
  where
    ...
    mkParamTy [] = Elaborable_Type_Universe fc
    mkParamTy (binder :: ps)
       = Elaborable_Dependent_Function_Type fc binder.rig binder.val.info (Just binder.name.val) binder.val.boundType (mkParamTy ps)
