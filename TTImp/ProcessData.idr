...
updateNS orig ns (Elaborable_Dependent_Function_Type fc c p n ty sc) = Elaborable_Dependent_Function_Type fc c p n ty (updateNS orig ns sc)
updateNS orig ns tm = updateNSApp tm
  where
    ...
    updateNSApp (Elaborable_Name fc n) -- data type type, must be defined in this namespace
        = if n == orig
             then Elaborable_Name fc ns
             else Elaborable_Name fc n
    updateNSApp (Elaborable_Apply fc f arg) = Elaborable_Apply fc (updateNSApp f) arg
    updateNSApp (Elaborable_Automatic_Apply fc f arg) = Elaborable_Automatic_Apply fc (updateNSApp f) arg
    updateNSApp (Elaborable_Named_Apply fc f n arg) = Elaborable_Named_Apply fc (updateNSApp f) n arg
    updateNSApp t = t
         ...
         ty <-
             wrapErrorC opts (InCon cn_in) $
                   checkTerm !(resolveName cn) InType opts nest env
                              (Elaborable_Bind_Here fc (PI erased) ty_raw)
                              (gType fc u)
