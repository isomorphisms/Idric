...
mutual
  ...
  unelabCase : {vars : _} ->
               ...
               Name ->
               List (Term vars) ->
               Core (Maybe Kinded_Elaborable_Term)
  unelabCase nest env n args
    ...
    where
      ...
      mkClause : FC -> Nat ->
                 ...
                 Core (Maybe Kinded_Elaborable_Clause)
      ...
      mkCase : List (vs ** (Env Term vs, Term vs, Term vs)) ->
               (argpos : Nat) -> List (Term vars) -> Core (Maybe Kinded_Elaborable_Term)
      mkCase pats argpos args
          = do unless (null args) $ log "unelab.case.clause" 20 $
               ...
               pure $ Just $ Elaborable_Case fc [] tm (Implicit fc False) pats'

  dropParams : List (Name, Nat) -> (Kinded_Elaborable_Term, Glued vars) ->
               Core (Kinded_Elaborable_Term, Glued vars)
  dropParams nest (tm, ty)
     = case getFnArgs tm [] of
            (Elaborable_Name fc n, args) =>
                case lookup (rawName n) nest of
                     Nothing => pure (tm, ty)
                     Just i => pure $ (apply (Elaborable_Name fc n) (drop i args), ty)
            _ => pure (tm, ty)
    where
      apply : Kinded_Elaborable_Term -> List Kinded_Elaborable_Argument -> Kinded_Elaborable_Term
      apply tm [] = tm
      apply tm (Explicit fc a :: args) = apply (Elaborable_Apply fc tm a) args
      apply tm (Auto fc a :: args) = apply (Elaborable_Automatic_Apply fc tm a) args
      apply tm (Named fc n a :: args) = apply (Elaborable_Named_Apply fc tm n a) args
  ...
  unelabTy' umode nest env (Local fc _ idx p)
      = do let nm = nameAt p
           ...
           pure (Elaborable_Name fc (MkKindedName (Just Bound) nm nm), ty)
  unelabTy' umode nest env (Ref fc nt n)
      = do defs <- get Ctxt
           Just ty <- lookupTyExact n (gamma defs)
               | Nothing => case umode of
                                 ImplicitHoles => pure (Implicit fc True, gErased fc)
                                 _ => pure (Elaborable_Name fc (MkKindedName (Just nt) n n), gErased fc)
           fn <- getFullName n
           n' <- case umode of
                      NoSugar _ => pure fn
                      _ => aliasName fn
           ...
           pure (Elaborable_Name fc (MkKindedName (Just nt) fn n'), gnf env (embed ty))
  unelabTy' umode nest env (Meta fc n i args)
      = do defs <- get Ctxt
           let mkn = nameRoot n
           def <- lookupDefExact (Resolved i) (gamma defs)
           let term = case def of
                              (Just (BySearch _ d _)) => Elaborable_Search fc d
                              _ => Elaborable_Hole fc mkn
      ...
      = do (fn', gfnty) <- unelabTy umode nest env fn
           ...
           case fnty of
                ...
                NBind _ x (Pi _ rig p ty) sc
                  => do sc' <- sc defs (toClosure defaultOpts env arg)
                        pure (Elaborable_Named_Apply fc fn' x arg',
                                glueBack defs env sc')
                _ => pure (Elaborable_Apply fc fn' arg', gErased fc)
  unelabTy' umode nest env (As fc s p tm)
      = do (p', _) <- unelabTy' umode nest env p
           ...
           case p' of
                Elaborable_Name _ n =>
                    case umode of
                         NoSugar _ => pure (Elaborable_As_Pattern fc (getLoc p) s n.rawName tm', ty)
                         _ => pure (tm', ty)
                _ => pure (tm', ty) -- Should never happen!
  unelabTy' umode nest env (TDelayed fc r tm)
      = do (tm', ty) <- unelabTy' umode nest env tm
           defs <- get Ctxt
           pure (Elaborable_Delayed_Type fc r tm', gErased fc)
  unelabTy' umode nest env (TDelay fc r _ tm)
      = do (tm', ty) <- unelabTy' umode nest env tm
           defs <- get Ctxt
           pure (Elaborable_Delay fc tm', gErased fc)
  unelabTy' umode nest env (TForce fc r tm)
      = do (tm', ty) <- unelabTy' umode nest env tm
           defs <- get Ctxt
           pure (Elaborable_Force fc tm', gErased fc)
  unelabTy' umode nest env (PrimVal fc c) = pure (Elaborable_Primitive_Value fc c, gErased fc)
  ...
  unelabTy' umode nest env (TType fc _) = pure (Elaborable_Type_Universe fc, gType fc (MN "top" 0))
  ...
  unelabPi : {vars : _} ->
             ...
             Core (PiInfo Kinded_Elaborable_Term)
  ...
  unelabBinder : {vars : _} ->
                 ...
                 Kinded_Elaborable_Term -> Term (x :: vars) ->
                 Core (Kinded_Elaborable_Term, Glued vars)
  unelabBinder umode nest fc env x (Lam fc' rig p ty) sctm sc scty
      = do (ty', _) <- unelabTy umode nest env ty
           ...
           pure (Elaborable_Lambda fc rig p' (Just x) ty' sc,
  ...
  unelabBinder umode nest fc env x (Let fc' rig val ty) sctm sc scty
      = do (val', vty) <- unelabTy umode nest env val
           ...
           pure (Elaborable_Binding fc EmptyFC rig x ty' val' sc,
  ...
  unelabBinder umode nest fc env x (Pi _ rig p ty) sctm sc scty
      = do (ty', _) <- unelabTy umode nest env ty
           ...
           pure (Elaborable_Dependent_Function_Type fc rig p' nm ty' sc, gType fc (MN "top" 0))
    where
...
unelabNoSugar : {vars : _} ->
                {auto c : Ref Ctxt Defs} ->
                Env Term vars -> Term vars -> Core Kinded_Elaborable_Term
