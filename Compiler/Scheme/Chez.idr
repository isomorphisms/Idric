  ...
  = fromString $
    ...
    (case (machine-type)
      ...
      [(i3le ti3le a6le ta6le tarm64le)
         ;; libc.so.6 is the glibc name, while Android exposes libc.so.
         ;; Unwind the failed load before trying the fallback: returning from
         ;; a handler for the non-continuable load exception raises
         ;; &non-continuable instead of continuing with the fallback.
         (guard (condition
